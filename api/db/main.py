...
def get_players(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    players = crud.get_all_players(db, skip=skip, limit=limit)
    return players
...
def create_player(new_player: player.PlayerCreate, db: Session = Depends(get_db)):
    # db_player = crud.get_player(db, player_id=player.Player.id)
    # if db_player:
    #     raise HTTPException(status_code=400, detail="Player already registered")
    return crud.new_player(db=db, new_player=new_player)
